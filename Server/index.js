...
app.get("/api/get", (req,res) =>{
    const sqlGet = "SELECT * FROM Contact_list";
...
app.post("/api/post", (req, res)=>{
    ...
    const addId = "Select max(Id)+1 as maxId from contact_list"
    let newId = 0
        ...
        const sqlInsert = "INSERT INTO contact_list (Id, fname,lname,org,position,phone,Other_phone,email,WC_resident,notes,Allow_photos,Want_info,Fallows_conversation,durrett_mtg,hobbies,skill_to_Share,address,Events_Attended, isDonor, Areas_of_Interest) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
...
app.delete("/api/remove/:Id", (req, res)=>{
    const { Id } =req.params;
    const sqlRemove = "DELETE FROM contact_list WHERE Id = ?";
...
app.get("/api/get/:Id", (req,res) =>{
    const{ Id } = req.params;
    const sqlGet = "SELECT * FROM Contact_list WHERE Id = ?";
...
app.put("/api/update/:Id", (req,res) =>{
    ...
    const sqlUpdate = "UPDATE Contact_list SET Fname=?,Lname=?,org=?,position=?,phone=?,Other_phone=?,email=?,WC_resident=?,notes=?,Allow_photos=?,Want_info=?,Fallows_conversation=?,durrett_mtg=?,hobbies=?,skill_to_Share=?,address=?,Events_Attended=?, isDonor=?, Areas_of_Interest=?  WHERE Id = ?";
...
app.post("/sort", (req, res) => {
    ...
    if (sortBy === "Events_Attended") {
      ...
      sqlGet =
        "SELECT *, LENGTH(Events_attended) - LENGTH(REPLACE(Events_attended, ',', '')) + 1 AS events_attended_count FROM Contact_list ORDER BY events_attended_count DESC";
    } else {
      sqlGet = `SELECT * FROM Contact_list ORDER BY ${sortBy}`; //sort by user input
    }
    db.query(sqlGet, (error, result) => {
      if (error) {
        console.log(error);
